...
for patch in patches:
    t = (
        tag
        if os.environ.get('SIM_SEED')
        else tag.replace('simSEED', f'sim{patch}')
    )
...
fig, axes = plt.subplots(
    1, len(gbins), figsize=(4.2 * len(gbins), 4), squeeze=False
)
for col, (glo, ghi) in enumerate(gbins):
    ...
    for i, state in enumerate(('trough_none', 'trough_forward')):
        ...
        print(
            f'  G {glo:4.1f}-{ghi:4.1f} {state:15s} n={w.sum():4d}: '
