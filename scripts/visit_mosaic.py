...
def main():
    ...
    cens = np.array([[r.getWcs().pixelToSky(r.getBBox().getCenterX(),
                      ...
                      r.getWcs().pixelToSky(r.getBBox().getCenterX(),
                                            r.getBBox().getCenterY()).getDec().asDegrees()]
                     for r in recs])
    ra0 = np.rad2deg(np.arctan2(np.mean(np.sin(np.deg2rad(cens[:, 0]))),
    ...
    for k, r in enumerate(recs):
        ...
        ny, nx = img.shape
        my, mx = ny // BIN, nx // BIN
        b = img[:my * BIN, :mx * BIN].reshape(my, BIN, mx, BIN).mean(axis=(1, 3))
        bg = bglist.getImage().array
        d = (img - bg)[:my * BIN, :mx * BIN].reshape(my, BIN, mx, BIN).mean(axis=(1, 3))
        wcs = r.getWcs()
        bb = r.getBBox()
        gy, gx = (np.mgrid[0:my, 0:mx] + 0.5) * BIN
        ra, dec = wcs.pixelToSkyArray((gx + bb.getBeginX()).ravel().astype('f8'),
    ...
    print(f'visit {visit}: sky median {med:.0f} nJy, range over the mosaic '
          f'{np.nanpercentile(sky, 1):.0f} .. {np.nanpercentile(sky, 99):.0f}')
    for name, arr, lo, hi, label in (
        ('sky', sky, med * 0.85, med * 1.15,
         'sky as observed, linear +-15 percent about the median'),
        ('delivered', np.arcsinh(dlv / (0.02 * med)), -1.0, 4.0,
         'delivered image, asinh stretch'),
    ):
        fig = plt.figure(figsize=(12, 12))
        ax = fig.add_axes([0.02, 0.02, 0.96, 0.94])
        ax.imshow(arr, vmin=lo, vmax=hi, cmap='gray', origin='lower',
                  extent=(-half / 3600, half / 3600, -half / 3600, half / 3600))
        ax.set_title(f'visit {visit} {band}: {label}; east left, north up '
                     f'(tangent plane, degrees)', fontsize=11)
        ax.invert_xaxis()
