"""
Gaia DR3 stars: the patch extracts and positions, and the visit extracts.
...
maps them to patch pixels.  A visit's detectors span ten or so tracts,
so the pooled per-visit template has its own extract, made from the
same refcat shards and conversion (lsst_starsub.cli.make_gaia) over the
bounding circle of the visit's detectors.
"""
import os
